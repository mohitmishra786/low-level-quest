...
const DiscussionSection = ({ problemId }) => {
    ...
    const fetchComments = async (discussionId) => {
        try {
            ...
            const response = await api.get(`/api/problems/discussions/${discussionId}/comments`);
    ...
    const handleCreateDiscussion = async (e) => {
        e.preventDefault();
        try {
            const response = await api.post(`/api/problems/${problemId}/discussions`, newDiscussion);
    ...
    const handleCreateComment = async (e, discussionId) => {
        e.preventDefault();
        try {
            const response = await api.post(`/api/problems/discussions/${discussionId}/comments`, {
                content: newComment
            });
        ...
        <div className="discussion-section">
            ...
            <div className="discussions-list">
                {discussions.length === 0 ? (
                    <div className="no-discussions">No discussions yet. Be the first to start one!</div>
                ) : (
                        ...
                        <div key={discussion.id} className="discussion-item">
                            <div className="discussion-header">
                                ...
                                <span className="discussion-meta">
                                    by {discussion.username} on {new Date(discussion.created_at).toLocaleDateString()}
                                </span>
                            </div>
                            ...
                            >
                                {selectedDiscussion === discussion.id ? 'Hide Comments' : `View Comments (${discussion.comment_count || 0})`}
                            </button>
                                ...
                                <div className="comments-section">
                                    {loadingComments[discussion.id] ? (
                                        ...
                                        <>
                                            ...
                                            <form className="new-comment-form" onSubmit={(e) => handleCreateComment(e, discussion.id)}>
                                                <textarea
