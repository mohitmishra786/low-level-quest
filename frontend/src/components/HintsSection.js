...
const HintsSection = ({ problemId }) => {
    ...
    if (hints.length === 0) return <div className="no-hints">No hints available for this problem.</div>;
                    ...
                    {hints.map((hint, index) => (
                        <div key={hint.id} className="hint-item">
                            <div className="hint-header">
                                ...
                                <button
                                    className={`reveal-hint-button ${revealedHints.has(hint.id) ? 'revealed' : ''}`}
