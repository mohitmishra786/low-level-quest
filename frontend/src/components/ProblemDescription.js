      ...
      if (trimmedLine.startsWith("- ")) {
        ...
        if (listType !== "ul") {
          if (currentList !== null) {
            elements.push(
              listType === "ul" ? (
                <ul key={`ul-${elements.length}`} className="problem-list">
                  {currentList}
                </ul>
              ) : (
                <ol key={`ol-${elements.length}`} className="problem-list">
                  {currentList}
                </ol>
              )
            );
        ...
        currentList.push(
          <li key={lineIndex}>{formatParagraph(trimmedLine.substring(2))}</li>
        );
      }
  ...
  const renderFormattedDescription = () => {
    ...
    return parts.map((part, index) => {
      if (part.type === "code") {
        ...
        const sections = part.content.split(
          /^(Example:|Requirements:|Note:|Input:|Output:)/gm
        );
        ...
        for (let i = 0; i < sections.length; i++) {
          if (i % 2 === 1) {
            ...
            sectionElements.push(
              <h3 key={`section-${i}`} className="section-header">
                {sections[i]}
              </h3>
            );
